...
MANIFEST_FILE_TEMPLATE = os.path.dirname (os.path.realpath(__file__)) + '/upload_manifest_template.xml'
...
class PanoptoUploader:
    ...
    def __create_manifest_for_video(self, file_path, manifest_file_name):
        ...
        with open(MANIFEST_FILE_TEMPLATE) as fr:
            template = fr.read()
        content = template\
        .replace('{Title}', Path (file_name).stem)\
    ...
    def __monitor_progress(self, upload_id):
        ...
        while True:
            ...
            if session_upload['State'] == 3: # Completed
                print ('Share ID: {0}'.format (session_upload['SessionId']))
                break
